...
impl Packets {
    pub fn new(
        ...
        let packet_dur = pkt_len as f32 / sample_rate as f32;
        let packet_dur_ms = (packet_dur * 1000 as f32) as u32;
    ...
    fn resovle_encoder_frame_sync(&mut self) {
        if self.seek_start_pkt_idx == 0 {
            return;
        }

        loop {
            let frame_idx_delta = self.packet_start_idx - self.sample_frames.get_curr_frame_idx();
        ...
        while self.packet_start_idx > self.sample_frames.get_curr_frame_idx() {
